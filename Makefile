...
IMPLS = ada awk bash basic c d chuck clojure coffee common-lisp cpp crystal cs dart \
	...
	haxe io java julia js kotlin logo lua make mal ocaml oli-clj matlab miniMAL \
...
oli_STEP_TO_PROG_clj  = oli-clj/target/$($(1)).jar
oli_STEP_TO_PROG_cljs = oli-clj/src/mal/$($(1)).cljc
...
oli-clj_STEP_TO_PROG = $(oli_STEP_TO_PROG_$(CLJ_MODE))
...
get_run_prefix = $(strip $(if $(strip $(DOCKERIZE)),\
    ...
    $(if $(filter oli-clj,$(1)),-e CLJ_MODE=$(CLJ_MODE),) \
